...
<form action="<?= base_url('index.php/main/peserta_save')?>" method="post">
<div class="form-group">
<input type="hidden" name="id" value="<?= $getPeserta[0]['id'] ?>">
<input type="hidden" name="id_acara" value="<?= $fbj_28_1[0]['id_acara'] ?>">
<input type="hidden" name="kelas" value="<?= ucfirst($kelas) ?>">
<label for="">Nama Peserta</label>
<input type="text" value="<?= $getPeserta[0]['nama_peserta'] ?>" name="nama_peserta" id="" class="form-control" placeholder="Tulis nama anda disini" aria-describedby="helpId">
</div>
<div class="form-group">
...
foreach($fbj_28_1 as $major_value){
    $total_ticket = $this->Main_model->join('ticket_detail','*',array(array('table'=>'ticket','parameter'=>'ticket.id=ticket_detail.ticket_id')),array('jurusan_id'=>$major_value['idJurusan'],'id_acara'=>$id_acara,'kelas'=>$kelas,'ticket_id'=>$getPeserta[0]['id']),'','','','ticket.id');
    $kuota = $major_value['kuota'];
    $sisa_kursi = $kuota-count($total_ticket);
    if (count($total_ticket)<$kuota)
    {
        ?>  
        <option value="<?= $major_value['idJurusan'] ?>" <?= $major_value['idJurusan']==@$total_ticket[0]['jurusan_id'] ? 'selected' : ''  ?> ><?= $major_value['major']." (".$sisa_kursi.")" ?></option>
        <?php 
    } 
}
?>
